...
export class EditDeleteSubaccountAdminStore {
  ...
  getSubaccountAdminInfo = ({ id, userId, groupId }) => {
    ...
    axios
      .get(`${PROXY_P1}/tenants/${id}/groups/${groupId}/admins/${userId}`)
      .then(res => {
        if (res.status === 200) {
  ...
  updateSubaccountAdmin = ({ id, closeModal, userId, getUsers, groupId }) => {
    ...
    axios
      .put(
        `${PROXY_P1}/tenants/${id}/groups/${groupId}/admins/${userId}`,
  ...
  deleteSubaccountAdmin = ({ id, closeModal, userId, getUsers, groupId }) => {
    ...
    axios
      .delete(`${PROXY_P1}/tenants/${id}/groups/${groupId}/admins/${userId}`)
      .then(res => {
        if (res.status === 200) {
          getUsers({ id, groupId })
          closeModal()
